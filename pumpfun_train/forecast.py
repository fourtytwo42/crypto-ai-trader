"""Forecasting utilities for pumpfun models."""
...
def to_neuralforecast_format(
    ...
    """Convert DataFrame to NeuralForecast format."""
    data = {
        "unique_id": unique_id,
        "ds": df[timestamp_col],
        "y": df[target_col],
...
def _extract_forecast_values(forecast: pd.DataFrame) -> np.ndarray:
    """Extract forecast values from NeuralForecast output."""
...
def _disable_model_logging(model: object) -> None:
    ...
    try:
        if getattr(model, "_pumpfun_logging_disabled", False):
            return
        ...
        setattr(model, "_pumpfun_logging_disabled", True)
    except Exception:
...
def forecast_next_horizon(
    ...
    """Forecast next horizon using NeuralForecast model."""
    if horizon <= 0:
